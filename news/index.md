...
- Added explicit `\value{}` documentation for the
  ...
  S3 method (and its subclass aliases including
  [`print.psu_sturman()`](https://rgempp.github.io/personnelSelectionUtility/reference/print.psu_utility.md)
  and
  [`print.psu_comparison()`](https://rgempp.github.io/personnelSelectionUtility/reference/print.psu_comparison.md)),
  following the CRAN reviewer’s request. The previous resubmission
  missed the value tag in the regenerated Rd file due to a stale roxygen
  block; this has been corrected.
...
- **Continuous utility**: Naylor-Shine (`naylor_shine`),
  ...
  intervention utility (`shp_utility`), Boudreau-style discounted
  utility (`boudreau_utility`), probation-period adjustments
  (`probation_adjustment`, `probation_utility`), and Sturman’s (2001)
  integrated comprehensive model (`sturman_comprehensive`).
- **SDy estimation**: percentile method (`sdy_percentile`), proportional
  rules (`sdy_proportional`), Raju-Burke-Normand coefficient of
  variation (`sdy_rbn`), CREPID-style activity decomposition
  (`sdy_crepid`), cost-accounting (`sdy_cost_accounting`),
  superior-equivalents (`sdy_superior_equivalents`), observed
  (`sdy_observed`).
- **Incremental validity**: restricted canonical validity for a fixed
  ...
  (`relative_weights`), Budescu dominance analysis
  (`dominance_analysis`), composite effect size (`composite_d`).
...
- **Selection systems**: compensatory top-down
  (`compensatory_selection`), conjunctive multiple-hurdle simulation
  (`multiple_hurdle_selection`, `multiple_hurdle_selection_staged`), and
  Ock-Oswald-style comparisons (`compare_selection_systems`,
  `compare_selection_systems_staged`).
- **Decision support**: multi-attribute utility
  (`multiattribute_utility`), Pareto frontiers (`pareto_frontier`,
  `utility_fairness_frontier`), adverse-impact ratio
  (`adverse_impact_ratio`), Monte Carlo uncertainty propagation
  (`utility_monte_carlo`), sensitivity grid (`sensitivity_grid`),
...
- **Reference helpers**: model taxonomy and argument glossary as data
  frames (`model_taxonomy`, `argument_glossary`).

### Tests and validation

- The Taylor-Russell univariate integral and the Thomas-Owen-Gunst
  multivariate integral are cross-validated against
  `scipy.stats.multivariate_normal` on a 6 x 6 grid (TR 1939) and a 3 x
  3 grid (TOG 1977). Discrepancies are below 1e-3 throughout.
- `correct_r_lawley` validated to reproduce Thorndike Case II in the k =
  2 case to machine precision.
...
- `reproductions`: reproduces the qualitative pattern of
  Sturman’s (2001) comprehensive cascade and Ock and Oswald’s (2018)
  compensatory vs. multiple-hurdle comparison, plus a worked example
  with
  [`sturman_comprehensive()`](https://rgempp.github.io/personnelSelectionUtility/reference/sturman_comprehensive.md).

### Continuous integration

- `R-CMD-check.yaml`: macOS / Windows / Ubuntu (devel, release,
  oldrel-1).
